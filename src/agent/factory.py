...
def build_agent_executor(
    ...
    Args:
        ...
        skills: Strategy ids to activate. When *None* falls back to
            ...
            ``DEFAULT_AGENT_SKILLS``.
        model_override: Optional chat model override for the current request.
    ...
    if model_override:
        fallback_models = [
            model for model in (getattr(config, "litellm_fallback_models", []) or []) if model != model_override
        ]
        config = replace(config, litellm_model=model_override, litellm_fallback_models=fallback_models)
