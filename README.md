...
| 回测 | AI 回测验证 | 自动评估历史分析准确率，方向胜率、止盈止损命中率；Web 回测页可直接从自选股下拉框选择股票 |
| **Agent 问股** | **策略对话** | **多轮策略问答，支持均线金叉/缠论/波浪等 11 种内置策略，可切换对话模型，左侧按股票名称/代码分组，未匹配代码的会话自动归入默认分组，Web/Bot/API 全链路** |
...
新版 WebUI 已升级为更适合日常操作的工作台布局：

- 全站 UI 统一对齐为更接近 Tailwind Plus Application UI 的后台风格：浅色优先、保留深色模式、统一卡片/表单/表格/空态/提示信息层级
- 左侧菜单导航，便于在首页、策略问股、回测、系统设置之间切换
- 顶部状态横条与统一页头，提升各页面层级感并突出当前工作区信息
- 升级版登录页，支持首登初始化密码与更清晰的认证提示
- 卡片式自选股工作台，可直接管理 `STOCK_LIST`、查看行情摘要、批量分析并快速跳转到报告或 AI 追问
- 自选股池前端已改为共享状态管理，避免在开发态刷新页面时重复拉取同一批行情数据
- 自选股池操作已收敛为“刷新实时价格”和“一键全部重新分析”，批量重跑前会先弹出确认框，避免误触。
- 自选股池卡片中的最后分析时间统一按东八区展示到分钟。
- 自选股池卡片会直接显示趋势判断与操作建议标签，并补充方向图标，避免重复占用中间卡片空间。
- 自选股池卡片进一步压缩为更高密度的监控视图，桌面宽屏可一行展示三张卡片。
- 首页点“详情”会直接弹出居中详情窗口，并内置 `历史记录 / 问股` 两个 tab；`AI 追问` 会默认进入当前股票专属的问股视图。
- 美股与 ETF 卡片标题会优先显示有效名称；当上游行情只返回代码时，前端与后端都会自动回退到更合理的股票名称。
- 系统设置中的 `STOCK_LIST` 已调整为只读提示，避免与首页选股工作台形成双入口维护
- 进行中的分析任务收纳到工作台顶部按钮，提示消息统一为右侧自动消失的 toast 弹窗
- 设置页改为单页分区：`AI 模型 / 视觉模型 / 数据源 / 代理设置 / 通知渠道 / 安全设置 / 定时任务 / 其他设置`
- AI 模型页修复为真实读写 `LITELLM_MODEL` / `LITELLM_FALLBACK_MODELS`，并可识别已注释保留的渠道配置
- 数据源与通知凭证支持“注释即停用”：停用时保留原值，重新启用时可直接恢复
- 密码登录与修改密码已集中到“安全设置”；定时任务配置已从通用系统项中单独拆出
- 首页、历史详情、回测页统一为摘要卡 + 数据区的工作台结构；设置页改为左侧分类导航 + 右侧分组管理台，更容易理解和维护
- AI 与通知配置按 `env.example` 的推荐方式重排为“最简单可用”模式：先看推荐方案，再填最少字段，最后再处理高级参数
- AI 模型页进一步简化为“顶部模型路由 + 下方品牌条目”的配置流程，新增模型时先选品牌，再在对应卡片里补真实配置路径
- AI 模型页现已改为“顶部模型路由 + 下方品牌条目”结构：顶部只保留 `LITELLM_MODEL` / `LITELLM_FALLBACK_MODELS` / 视觉模型设置，下面按 `.env.example` 的真实配置路径展示 `DeepSeek / Gemini / OpenAI / Qwen / GLM / Moonshot / MiniMax` 等品牌卡片
- 通知渠道页已将 `REPORT_*` 收口为一个“报告设置”按钮；首屏只展示带图标的缩略摘要卡片，详细配置则进入独立面板查看
- 通知渠道页也改为极简模式：默认空白，只展示已配置渠道，可按需添加，并通过启用/停用方式管理是否保留该渠道配置
- 策略回测页的筛选输入区与执行按钮已重构为独立控制台，统一输入框、按钮和卡片风格
- 首页全局历史面板已移除，改为按股票查看独立历史详情页：左侧历史记录，右侧分析报告内容
- 添加选股入口已收口为单一按钮，点击后弹窗支持手动输入股票代码或从图片中提取股票
- 自选股池卡片已优化，涨跌颜色标记更明显，卡片信息层级与操作区也做了精细化调整
- 首页头部已移除“输入代码快速分析 / 立即分析”控件，改为只保留更统一的“添加选股”入口
- 策略问答已重构为三栏布局：最左侧股票代码、中间历史对话、右侧问答区域，策略选择改为下拉框形式
- 设置页文案与说明进一步压缩，只保留必要提示；代理、Vision、定时任务等配置已按用途拆分
...
发布到 Docker WebUI 前，建议以上两步都通过，并确认最新静态产物已输出到 `static/`。
