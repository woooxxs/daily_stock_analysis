...
- 配置区重构为更清晰的“配置中心”布局，增强分类导航、字段提示、敏感项编辑提示与 LLM 渠道管理体验
- 系统设置页新增“密码登录管理”：可在设置页内直接开启/关闭管理员密码登录、首次设置密码，并在关闭认证时通过弹窗确认操作
- 系统设置页重新开放“数据源”分类，并补充数据接口优先级、Vision 图片识股、代理 / 抓取兼容等更贴近实际部署的配置说明
...
- 系统设置页已进一步收敛，只保留普通用户真正需要维护的系统项，隐藏了日志、端口、代理、调试等偏后台配置
